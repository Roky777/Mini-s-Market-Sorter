import { createGame } from "./core/game.js?v=20260924-certificate-layout-1";
...
const game = createGame({ persistProgress: !isDebugLaunch, gameId: "Mini-s-Market-Sorter" });
let sparky;
let successDance;
...
if (Number.isInteger(requestedLevel) && requestedLevel >= 1 && requestedLevel <= levelCount) {
...
const successScreen = {
  ...
  list() {
    const commands = Array.from({ length: levelCount }, (_, index) => ({
      level: index + 1,
...
if (Number.isInteger(requestedSuccessLevel) && requestedSuccessLevel >= 1 && requestedSuccessLevel <= levelCount) {
