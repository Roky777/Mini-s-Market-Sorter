...
export function renderGameUi(state, level) {
  ...
  if (state.screen === "complete") {
    ...
    screen.setAttribute("aria-label", "Mini's Market Master certificate");
    ...
    card.innerHTML = `
      ...
      <div class="game-certificate__results" aria-label="Campaign results">
        <div class="game-certificate__score"><small>Levels</small><strong><i aria-hidden="true">✓</i>${MATH_LEVELS.length}<b>/${MATH_LEVELS.length}</b></strong></div>
        <div class="game-certificate__stat game-certificate__stat--xp"><small>XP earned</small><strong>${formatXp(state.campaignXp)}<b>/200</b></strong></div>
  ...
  prompt.innerHTML = `<span>Level ${state.level} of ${MATH_LEVELS.length}</span><h1>${level.title}</h1><p>${level.instruction}</p>`;
  root.append(prompt);
  ...
  if (state.completedLevel) {
    ...
    success.innerHTML = `
      <img class="success-screen__background" src="assets/backgrounds/market_blur.webp" alt="" />
    ...
    next.innerHTML = `<span>${state.level === MATH_LEVELS.length ? "Finish" : "Next Level"}</span><i aria-hidden="true">›</i>`;
    actions.append(next);
    root.append(success);
    return;
  }
  if (state.paused) {
    ...
    pause.innerHTML = `
      <img class="pause-screen__background" src="assets/backgrounds/market_blur.webp" alt="" />
