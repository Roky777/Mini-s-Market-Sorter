...
import { TutorialController } from "../tutorial/tutorial-controller.js?v=20260924-complete-bin-1";
...
export function createGame({ persistProgress = true, gameId = "Mini-s-Market-Sorter" } = {}) {
  ...
  function choose(itemId, category) {
    ...
    if (correct) {
      ...
      const xpGain = firstTry
        ? getObjectXp(state.levelIndex + 1, MATH_LEVELS.length, state.completedMastery + 1, state.totalRequired)
        : 0;
      analytics.recordTask({
      ...
      if (state.levelIndex === 2 && previousInCategory && previousInCategory !== activeItem.name && !state.specialRewards.includes("market-match")) {
        state.specialRewards.push("market-match");
        rewardTitle = "Market match!";
      }
      ...
      advanceTimer = window.setTimeout(() => {
        ...
        state.feedback = {
          ...
          message: xpGain > 0
            ...
            : `${rewardTitle ?? (newlyMastered ? "Mastered!" : "Correct!")} Keep going!`,
          category,
        };
        sounds.success();
